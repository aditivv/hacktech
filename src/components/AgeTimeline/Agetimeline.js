...
import Supabase, { addPerson, updatePerson, addTech, removeTech, getConfidence, getAdaptability, getAttentionSpan, getImpulsivity, getIrritability, getName, getAgeTechIntro } from "../../supabaseClient.js";
...
export default function AgeTimeline({ ages, currentAge, onAgeDown, onAgeUp }) {
  ...
  const ageUp = async () => {
    ...
    for (let i = 0; i < TABLES.length; i++) {
      ...
      updatePerson(TABLES[i], currentAge + 1, res["confidence"], res["attention_span"], res["irritability"], res["impulsivity"], res["adaptability"])
      setProgress(i + 1);
    }
